        ...
        return array(
            ...
            array('isEnable,memberPointPayPreStoreLimit, memberTotalPayPreStoreLimit', 'numerical'),
             array('isEnable,memberPointPayPreStoreLimit, memberTotalPayPreStoreLimit', 'match', 'pattern' => '/^\d+(\.\d{1,2}){0,1}$/u',
                'message' => '只能保留两位小数！'),
            array('memberPointPayPreStoreLimit','checkMoney')
        );
    }
    
     public function checkMoney($attribute, $params){       
         if($this->memberPointPayPreStoreLimit > $this->memberTotalPayPreStoreLimit){
             $this->addError($attribute, Yii::t('goods', '积分限额不可以大于总消费限额'));
         }
     }

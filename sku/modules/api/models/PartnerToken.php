...
{
	const CACHE_PATH = 'skuApiPartnerTokenCache';
...
// 					$memberInfo = Member::getMemberInfoByGaiId($memberData['id']);
// 					if (!empty($memberInfo)) {
...
// 			}
    ...
    public static function getPartnersInfo($where,$fields = "*"){
        ...
        $result = Yii::app()->db->createCommand()->select($fields)->from("{{partner_token}}")->where($whereStr,$whereVal)->queryRow();
        return $result;
    }
}

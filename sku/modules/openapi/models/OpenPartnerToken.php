 ...
 */
class OpenPartnerToken extends CActiveRecord
{
	const CACHE_PATH = 'skuOpenPartnerTokenCache';
	...
	public function rules()
	{
		...
		return array(
			array('member_id,private_key,public_key', 'required'),
			...
			array('id, member_id, token, create_time, expir_time,lang,private_key,public_key', 'safe', 'on'=>'search'),
		);
	}
...
// 			'member' => array(self::BELONGS_TO, 'GwMember', 'member_id'),
	...
	public static  function destoryToken($member_id)
	{
		...
		return Yii::app()->db->createCommand(' DELETE  FROM  '.self::model()->tableName().' WHERE member_id='.$member_id)->execute();
	}

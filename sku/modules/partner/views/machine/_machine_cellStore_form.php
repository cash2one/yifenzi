    ...
    <tbody>
        <tr>
            ...
            <td>
                ...
                <?php }else{;?>
                <input class="inputtxt1" id="RefGoodsName" name="RefGoodsName" style='width:300px;'
                       readonly="readonly" type="text" value="<?php echo $name;?>" />
                <?php } ?>
                ...
                <script type="text/javascript">
                    $("#clearRefMem").click(function(){
                        $("#RefGoodsName").val('');
                        $("#VendingMachineCellStore_goods_id").val('');
                    });
                </script>
...
<!--            --><?php //echo $form->error($model, 'stock'); ?>
    ...
    <tr>
        <th width="10%"><?php echo Yii::t('partnerModule.machine','格子铺编码'); ?><span class="required">*</span></th>
        <td width="90%"><?php echo $form->textField($model, 'code', array('class' => 'inputtxt1','style'=>'width:300px;')); ?> <?php echo $form->error($model,'code') ?></td>
...
<div class="profileDo mt15">
    <a href="javascript:void(0);" class="sellerBtn03" onclick="$('#machine-form').submit();"><span><?php echo $model->isNewRecord?Yii::t('partnerModule.machine', '添加'):Yii::t('partnerModule.machine', '修改');  ?></span></a>&nbsp;&nbsp;<a href="javascript:history.go(-1);" class="sellerBtn01"><span><?php echo Yii::t('partnerModule.machine','返回'); ?></span></a>
</div>
...
jQuery(function($) {
    $('#seachRefMem').click(function() {
        dialog = art.dialog.open('" . $this->createAbsoluteUrl('goods/searchList',array('type'=>Stores::MACHINE,'sid'=>$mid)) . "', { 'id': 'selectGoods', title: '".Yii::t('partnerModule.machine','搜索商品')."', width: '1000px', height: '520px', lock: true });
    })
})
...
var onSelectGoods = function (id,name,thumb,spec_id) {
    if (id) {
        $('#VendingMachineCellStore_goods_id').val(id);

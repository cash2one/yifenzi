...
/**
把$cs->registerScriptFile($baseUrl."/js/swf/js/handlers.js"); 写到了文件里
为了英文化 。
...
function fileQueued(file) {
	try {
		...
		progress.setStatus("<?php echo Yii::t('sellerUpload','等待上传...')?>");
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("<?php echo Yii::t('sellerUpload',"对不起，每次最多允许选择") ?>"+message+"<?php echo Yii::t('sellerUpload',"个文件")?>");
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			progress.setStatus("<?php echo Yii::t('sellerUpload','文件太大.')?>");
			this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			progress.setStatus("<?php echo Yii::t('sellerUpload','不允许上传0字节的文件.')?>");
			this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			progress.setStatus("<?php echo Yii::t('sellerUpload','未知文件类型.')?>");
			this.debug("Error Code: Invalid File Type, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		default:
			if (file !== null) {
				progress.setStatus("<?php echo Yii::t('sellerUpload','未知错误！')?>");
			}
...
function uploadStart(file) {
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("<?php echo Yii::t('sellerUpload','上传中...')?>");
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		if (serverData.indexOf("suc")=='0') {//如果提示成功
			...
			var status = "<?php echo Yii::t('sellerUpload','恭喜你，文件上传成功！')?> <br />";
			...
			progress.setError();
			var endnum = serverData.indexOf("|");
			progress.setStatus("<?php echo Yii::t('sellerUpload','上传失败：')?>"+serverData.substring(3,endnum));
			progress.toggleCancel(false);
		}
...
//		alert(message+'======'+errorCode);
		...
		case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
			progress.setStatus("<?php echo Yii::t('sellerUpload','文件上传失败: ')?>" + message);
			this.debug("Error Code: HTTP Error, File name: " + file.name + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
			progress.setStatus("<?php echo Yii::t('sellerUpload','文件上传失败')?>");
			this.debug("Error Code: Upload Failed, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.IO_ERROR:
			progress.setStatus("<?php echo Yii::t('sellerUpload','服务器IO错误')?>");
			this.debug("Error Code: IO Error, File name: " + file.name + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
			progress.setStatus("<?php echo Yii::t('sellerUpload','安全错误')?>");
			this.debug("Error Code: Security Error, File name: " + file.name + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			progress.setStatus("<?php echo Yii::t('sellerUpload','文件大小超过限制')?>");
			this.debug("Error Code: Upload Limit Exceeded, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
			progress.setStatus("<?php echo Yii::t('sellerUpload','验证失败，上传已被跳过')?>");
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			progress.setStatus("<?php echo Yii::t('sellerUpload','已取消上传')?>");
			progress.setCancelled();
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			progress.setStatus("<?php echo Yii::t('sellerUpload','已停止上传')?>");
			break;
		default:
			progress.setStatus("<?php echo Yii::t('sellerUpload','未知错误: ')?>" + errorCode);
			this.debug("Error Code: " + errorCode + ", File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
			break;
		}
	} catch (ex) {
...
//	if(linum==5)$('.list').addClass("overdiv");
...
function queueComplete(numFilesUploaded) {
	...
	status.innerHTML = numFilesUploaded + "<?php echo Yii::t('sellerUpload',' 个文件被上传.')?>";
}
...
<script type="text/javascript">
	...
	window.onload = function() {
		var settings = {
			...
			upload_url: "<?php echo Yii::app()->createUrl('/partner/upload/upload');?>",	// Relative to the SWF file
			post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","YII_CSRF_TOKEN":'<?php echo Yii::app()->request->csrfToken;?>',"uid":'<?php echo Yii::app()->user->id ?>'},
			file_size_limit : "2 MB",
			file_types : "<?php echo $img_format?>",
			file_types_description : "<?php echo Yii::t('sellerUpload', '图像'); ?>",
			...
			button_text_style: ".redText{color:#FFFFF;font-size:14px;}",
	...
	<div class="fieldset flash" id="fsUploadProgress"><span class="legend"><?php echo Yii::t('sellerUpload', '快速上传'); ?></span></div>
		<div id="divStatus"><?php echo Yii::t('sellerUpload', '0 个文件已上传'); ?></div>
	<div>
		<span id="spanButtonPlaceHolder"></span>
		<input id="btnUpload" type="button" value="<?php echo Yii::t('sellerUpload', '点此开始上传'); ?>" onclick="swfu.startUpload();"  disabled="disabled" style="margin-left: 2px; width:121px; height: 25px; cursor:pointer;" />
		<input id="btnCancel" type="button" value="<?php echo Yii::t('sellerUpload', '取消所有上传'); ?>" onclick="swfu.cancelQueue();" disabled="disabled" style="margin-left: 2px; font-size: 8pt; height: 29px;" />
	</div>

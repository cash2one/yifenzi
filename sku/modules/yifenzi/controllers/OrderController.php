...
//                print_r($this->getPost('pay_type'));exit;
                ...
                //订单数据
                $sql = "select order_id,order_sn,member_id,order_amount,order_channel from " . YIFENZI . '.gw_yifenzi_order where order_id='.$this->getPost('orderID');
                ...
                switch( $this->getPost('pay_type') )
                {
                    case 'pay_1':
                        ...
                        echo (json_encode(array(
                            "err" => 2,
                            'order_channel' => $OrderData['order_channel'],
                            "msg" => "支付成功",
                        )));
                        exit();
                        break;
...
//        $order_sn = $getData['order_sn'];
        ...
        $sql = "select order_id,order_sn,member_id,order_amount,order_channel from " . YIFENZI . ".gw_yifenzi_order where order_id={$order_id}";
        $OrderData = Yii::app()->db->createCommand( $sql )->queryRow();
        if (!$orderGoodsName) {
...
//            $input->SetNotify_url('http://www.g1fz.com/order/wxnotifyurl');
            $input->SetNotify_url(DOMAIN_YIFENZI.Yii::app()->createUrl('/yifenzi/order/wxnotifyurl'));
            ...
            if ( $jsApiParameters ) {
                ...
                //订单商品详情
                $orderGoodsData = $this->getOrderGoodsInfo($order_id,Yii::app()->user->id);
                if ( !$orderGoodsData )
                ...
                $this->render('topayment',
                    array('PayDataConfig'=>$PayDataConfig,
                        "ordergoods"=>$orderGoodsData,
...
//                throw new Exception('订单支付有误，请联系客服人员');
            ...
            $apiLogData['node'] = isset($post['node']) ? $post['node'] : AccountFlow::BUSINESS_NODE_EBANK_WEIXIN;
    ...
    public function actionReturnurl(){
        ...
        switch( $pay_result ){
            case 1:
                if ($data['order_channel'] == 2){
                    $url = DOMAIN_YIFENZI2."/order/ordersuccess/code/".$data['order_id']."_2/msg/支付成功";
                    header("Location:$url");
                    exit();
                }

                if ($data['order_channel'] == 3){
                    $url = DOMAIN_YIFENZI3."/order/ordersuccess/code/".$data['order_id']."_2/msg/支付成功";
                    header("Location:$url");
                    exit();
                }
            ...
            case 2:
                if ($data['order_channel'] == 3){
                    $url = DOMAIN_YIFENZI3."/order/ordersuccess/code/".$data['order_id']."_1/msg/支付失败";
                    header("Location:$url");
                    exit();
                }

                if ($data['order_channel'] == 2){
                    $url = DOMAIN_YIFENZI2."/order/ordersuccess/code/".$data['order_id']."_1/msg/支付失败";
                    header("Location:$url");
                    exit();
                }
    ...
    public function actionTopayment(){
        ...
        $types = Yii::app()->request->getParam("types") ? intval(Yii::app()->request->getParam("types")) : 0;

        if ($types){
            $sql = "select member_id from {{order}} where order_id={$order_id}";
            $memberData = Yii::app()->gwpart->createCommand($sql)->queryRow();
        ...
        //验证数据,是否有此订单
        $orderGoodsData = $this->getOrderGoodsInfo($order_id ,$member_id);
     ...
     * @return mixed
     */
    public function getOrderGoodsInfo($order_id ,$member_id=0){
        if(!$member_id){
...
//            throw new Exception('操作有误，请联系客服人员。');
            try{
                $member_id = Yii::app()->user->id;
                $sql = "select id,gai_number,gai_member_id,password3,salt from {{member}} where id = $member_id";
                $skuMember = Yii::app()->db->createCommand( $sql )->queryRow();
                if ( !$skuMember )
                    throw new Exception('未知错误，请联系管理人员.');

                if ( !$skuMember['password3'] ){
                    //校验时先同步Gw数据到Sku
//                    Member::syncPassword($skuMember['gai_number']);
                    $apiMember = new ApiMember;

                    $apiMemberData = $apiMember->getGWPassword($skuMember['gai_member_id']);
                    if (is_array($apiMemberData)){
                        $skuMember['password3'] = $apiMemberData['password3'];
                        $skuMember['salt'] = $apiMemberData['salt'];
                    }
                }

                //盖像会员数据
//                $sql = "select * from {{member}} where id=".$skuMember['gai_member_id'];
//                $gwMember = Yii::app()->db->createCommand( $sql )->queryRow();
                if ( $skuMember['password3'] == false ){

<header class="normal">
    ...
    <a href="<?php echo Yii::app()->createUrl('/yifenzi/goods/periods',array('id'=>$model->goods_id))?>" class="classify">第<?php echo Yii::app()->request->getParam('nper') ? Yii::app()->request->getParam('nper'): $model->current_nper?>期<span></span></a>
</header>
<?php 
...
<div class="container">
    ...
    <p class="personalAddress"><?php
        $address = Address::model()->find('member_id=:id AND `default`=:default',array(':id'=>$member['id'],':default'=>Address::DEFAULT_IS));
    ...
    <div class="award-btnGroup">
        <a href="<?php echo Yii::app()->createUrl('/yifenzi/goods/winningAll',array('id'=>$model->goods_id,'nper'=>$winning['current_nper'],'winning_code'=>$winning['winning_code']));?>" class="awardBtn award-num"><?php echo $winning['winning_code']?></a>
        <a href="<?php echo Yii::app()->createUrl('/yifenzi/goods/record',array('id'=>$model->goods_id,'nper'=>$winning['current_nper']));?>" class="awardBtn award-check">参与记录</a>
        <a href="<?php echo Yii::app()->createUrl('/yifenzi/goods/cloud',array('nper'=>$winning['current_nper'],'id'=>$model->goods_id))?>" class="awardBtn award-check">云计算方式</a>
    </div>
    <!-- <ul class="userInfo">
...
<div class="h60"></div>
<a href ="<?php echo Yii::app()->createUrl('/yifenzi/goods/view',array('id'=>$model->goods_id,'nper'=>$model->current_nper))?>">

...
//            array('username', 'match', 'pattern' => '/^GW\d+$/', 'message' => Yii::t('YifenziModule.member', '请输入正确的盖网号')),
            array('username', 'match', 'pattern' => '/[(^GW\d+)|(^1\d{10})]$/', 'message' => Yii::t('Yifenzi3Module.member', '请输入正确的盖网号或手机号')),
        );
    }
    ...
    {
        return array(
            'username' => Yii::t('Yifenzi3Module.member', '用户名'),
            'password' => Yii::t('Yifenzi3Module.member', '密码'),
            'verifyCode' => Yii::t('Yifenzi3Module.member', '验证码')
        );
    }
        ...
        if (!$this->hasErrors()) {
            ...
            if (!$this->_identity->authenticate()) {
                if (empty($this->_identity->errorMessage)) {
                    $this->addError('password', Yii::t('Yifenzi3Module.home', '用户名或密码错误'));
                } else {

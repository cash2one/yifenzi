        ...
        setTimeout(function () { // 插入数据
            if (!refresh) {//第一次插入-----根据高度判断插入数
                ...
                foreach ($lists as $k => $an) {
                    $content .= '<li><a href="' . Yii::app()->createUrl('/yifenzi3/goods/view', array('id' => $an['goods_id'],'nper'=>$an['current_nper'])) . '"><img src="' . ATTR_DOMAIN . '/' . $an['goods_thumb'] . '"></a>' .
                        '<div class="right">' .
                        '<p class="name"><span>[第' . $an['current_nper'] . '期]</span><a href="' . Yii::app()->createUrl('/yifenzi3/goods/view', array('id' => $an['goods_id'],'nper'=>$an['current_nper'])) . '">' . $an['goods_name'] . '</a></p>' .

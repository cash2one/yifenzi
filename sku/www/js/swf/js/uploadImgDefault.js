 ...
 */
function _fileUpload(openUrl,uploadUrl,picNum,oid,csrfToken,PHPSESSID){
	art.dialog.open(openUrl,{
		...
		ok:function(){
			...
			if(num>0){
				if(cache_num>num){
					alert(L.onlySelect+num+ L.picNumber);
					return false;
				}
			}
			...
			if(imgsData.length>0){		//如果有数据就将数据上传
				...
				jQuery.ajax({
					...
					url:uploadUrl,
					data:"imgdata="+encodeURIComponent(imgsData)+"&YII_CSRF_TOKEN="+csrfToken+"&PHPSESSID="+PHPSESSID,
					...
					success:function(data){
						...
						for(var i=0;i<data.length;i++){
							...
							html+= "<a href='javascript:;' onclick='delImgById(this,\""+data[i]['path']+"\",\""+oid+"\")' class='item_close'>"+ L['delete']+"</a>";

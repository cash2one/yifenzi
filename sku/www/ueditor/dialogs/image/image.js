...
(function() {
    ...
    function insertSingle() {
        ...
        if (!url.value)
            return;
        if (!flagImg)
            return;   //粘贴地址后如果没有生成对应的预览图，可以认为本次粘贴地址失败
                ...
                if (id == "imgManager") {
                    ...
                    if (!list.children.length) {
                        ajax.request(editor.options.imageManagerUrl, {
                            timeout: 100000,
                            action: "get",
                            dir: window.parent.UEDITOR_CONFIG.savePath,
                            "YII_CSRF_TOKEN": window.parent.UEDITOR_CONFIG.csrfToken,
                            onsuccess: function(xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++]; ) {
                                    ...
                                    img.ondblclick = function() {
                                        if (confirm('真的要删除这张图片吗？' + "\r\n" + this.src)) {
                                            this.parentNode.removeChild(this);
                                            ajax.request(editor.options.imageDeleteUrl, {
                                                action: "del",
                                                file: this.src,
                                                "YII_CSRF_TOKEN": window.parent.UEDITOR_CONFIG.csrfToken,
                                                onsuccess: function(xhr) {
                                                    if(xhr.responseText != 1) alert('删除图片失败');
                                                }
                                            });
                                        }
                                    }

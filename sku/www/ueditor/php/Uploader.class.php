...
class Uploader
{
    ...
    private function upFile( $base64 )
    {
        ...
        if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
            if (!UPLOAD_REMOTE  ) {
                if(!move_uploaded_file( $file[ "tmp_name" ] , $this->fullName )) $this->stateInfo = $this->getStateInfo( "MOVE" );
            }
            if(UPLOAD_REMOTE){
                $ftp = Yii::app()->ftp;
                $ftp->createDir(dirname($this->fullName));
                $ftp->put($this->fullName,$file[ "tmp_name" ]);
            }
        }
    }
...
//        file_put_contents(Yii::app()->basePath.'/xx.txt',$pathStr);exit;
        //$pathStr.=
        if (!UPLOAD_REMOTE && !file_exists( $pathStr ) ) {
